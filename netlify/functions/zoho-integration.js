...
let stockLockStorePromise = null;
let catalogStorePromise = null;
let checkoutStorePromise = null;
let catalogBlobHydrated = false;

async function getBlobModule() {
  if (!blobModulePromise) {
    blobModulePromise = import('@netlify/blobs').catch(error => {
      blobModulePromise = null;
      const e = new Error('Netlify distributed checkout storage is unavailable.');
      e.statusCode = 503;
      e.cause = error;
      throw e;
    });
...
async function getStockLockStore() {
  if (!stockLockStorePromise) {
    stockLockStorePromise = getBlobModule().then(({ getStore }) => getStore({ name: 'vestige-stock-locks', consistency: 'strong' }));
  }
  return stockLockStorePromise;
}
async function getCatalogStore() {
  if (!catalogStorePromise) {
    catalogStorePromise = getBlobModule().then(({ getStore }) => getStore({ name: 'vestige-product-map', consistency: 'strong' }));
  }
  return catalogStorePromise;
}
async function getCheckoutStore() {
  if (!checkoutStorePromise) {
    checkoutStorePromise = getBlobModule().then(({ getStore }) => getStore({ name: 'vestige-checkouts', consistency: 'strong' }));
  }
  return checkoutStorePromise;
}
...
async function hydratePersistedCatalog() {
  if (catalogBlobHydrated) return;
  catalogBlobHydrated = true;
  try {
    const store = await getCatalogStore();
    const data = await store.get('bc10000-v1', { type: 'json', consistency: 'strong' });
    if (!data || typeof data !== 'object') return;
    for (const flavour of Object.keys(PRODUCT_NAMES)) {
      const id = String(data[flavour] || '').trim();
      if (/^\d+$/.test(id) && !resolvedProductItemIds.has(flavour)) resolvedProductItemIds.set(flavour, id);
    }
  } catch (error) {
    console.warn('Unable to hydrate persisted Zoho product mapping', { message: error.message });
  }
}
async function persistCatalogMapping() {
  try {
    const store = await getCatalogStore();
    const mapping = Object.fromEntries(Object.keys(PRODUCT_NAMES).map(flavour => [flavour, resolvedProductItemIds.get(flavour) || null]));
    await store.setJSON('bc10000-v1', mapping);
  } catch (error) {
    console.warn('Unable to persist Zoho product mapping', { message: error.message });
  }
}
async function acquireStockLock(itemId, ownerId) {
  const store = await getStockLockStore();
...
function getAccountsUrl() {
  const raw = process.env.ZOHO_ACCOUNTS_URL || 'https://accounts.zoho.com';
...
function getExpectedBrowserOrigins(event) {
  ...
  for (const raw of [process.env.ALLOWED_ORIGIN, process.env.URL, process.env.DEPLOY_PRIME_URL, process.env.DEPLOY_URL]) {
    if (!raw) continue;
...
async function zohoRequest(path, { method = 'GET', body } = {}) {
  return withZohoSlot(async () => {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
      ...
      if (!response.ok || data.code !== 0) { console.error('Zoho Books API request failed', { status: response.status, code: data.code, method: normalizedMethod }); const e = new Error('Zoho Books API request failed.'); e.statusCode = response.status >= 500 ? 503 : 502; throw e; }
      return data;
    }
...
function locationStockReading(location) {
  // Prefer Zoho's explicit available-for-sale fields. Some Books organizations expose
  // only location_stock_on_hand even when inventory tracking is active; Zoho currently
  // documents that field as the current stock available at the location, so it is the
  // compatibility fallback rather than treating a blank available field as zero.
...
function locationAvailableStock(location) {
  return locationStockReading(location).value;
}
...
function chooseStockLocation(item, requestedQuantity = 1) {
  const configuredLocationId = String(process.env[STOCK_LOCATION_ID_ENV] || '').trim();
  const activeLocations = (Array.isArray(item?.locations) ? item.locations : [])
    .filter(location => String(location?.status || 'active').toLowerCase() !== 'inactive')
    .map(location => {
      const reading = locationStockReading(location);
      return {
        location,
  ...
  return { location: null, locationId: '', available, stockSource, physical, isPrimary: false };
}
...
async function discoverProductCatalog(force = false) {
  ...
  for (const flavour of Object.keys(PRODUCT_NAMES)) {
    ...
    const configuredId = envName ? String(process.env[envName] || '').trim() : '';
  ...
  if (!unresolved.length) { cachedProductCatalogUntil = Date.now() + PRODUCT_CATALOG_CACHE_MS; await persistCatalogMapping(); return; }
...
async function requireDeliveryItem() {
  ...
  const configuredId = String(process.env[DELIVERY_ITEM_ID_ENV] || '').trim();
...
async function confirmInventoryReservation(flavour, itemId, quantity, beforeSnapshot) {
  const beforeSignals = beforeSnapshot?.stockSignals || {};
  const beforeAvailable = Number(beforeSnapshot?.stock);
  ...
  for (const delay of delays) {
    ...
    let snapshot = buildStockSnapshot(flavour, item, 1);
    if (locationId) {
      const location = (Array.isArray(item.locations) ? item.locations : []).find(l => String(l.location_id || '') === locationId);
      if (location) {
        const available = locationAvailableStock(location);
        const physical = locationPhysicalStock(location);
        snapshot = {
          ...snapshot,
          stock: available === null ? snapshot.stock : Math.max(0, Math.floor(available)),
          physicalStock: physical === null ? snapshot.physicalStock : Math.max(0, Math.floor(physical)),
          locationId,
          stockSignals: collectStockSignals(item, locationId),
        };
      }
    }
    lastSnapshot = snapshot;
    const afterSignals = snapshot.stockSignals || {};
    const sameFieldDecrease = Object.keys(beforeSignals).some(field => {
      const before = Number(beforeSignals[field]);
      const after = Number(afterSignals[field]);
      return Number.isFinite(before) && Number.isFinite(after) && after <= before - Number(quantity);
    });
    const availableDecrease = Number.isFinite(beforeAvailable) && Number(snapshot.stock) <= beforeAvailable - Number(quantity);
    const noNegativeSignals = Object.values(afterSignals).every(v => !Number.isFinite(Number(v)) || Number(v) >= 0);
    if ((sameFieldDecrease || availableDecrease) && noNegativeSignals) return snapshot;
  }
  const e = new Error('Zoho did not confirm the inventory reduction for this order. The transaction was cancelled before payment.');
  e.statusCode = 409;
  e.freshAvailabilityNeeded = true;
  e.lastStockSnapshot = lastSnapshot;
  throw e;
}
...
exports.handler = async function handler(event) {
  ...
  // Accept both Netlify's rewritten path and canonical function path. Different
  // Netlify runtimes can expose either value in event.path after a rewrite.
  // Browser-origin validation and server-side validation remain authoritative.
  const acceptedPaths = new Set(['/api/zoho', '/.netlify/functions/zoho-integration']);
  if (event.path && !acceptedPaths.has(event.path)) return publicError(404, 'Not found.', requestId);
  ...
  try {
    ...
    if (body.action === 'connection_test') {
      ...
      return json(200, { success: true, message: 'Secure Zoho Books connection and BC10000 stock mapping confirmed.', organization: { organizationId, organizationName: org.name || null }, stockConnection, deliveryItem: { verified: true, name: deliveryItem.name || DELIVERY_ITEM_NAME, rate: Number(deliveryItem.rate) }, requestId });
    }
    ...
    try {
      ...
      if (!salesOrder) {
        ...
        beforeSnapshot = finalStock.snapshot;
        await renewDistributedLock(stockLock);
      ...
      if (createdSalesOrderThisAttempt && beforeSnapshot) {
        await confirmInventoryReservation(order.flavour, productItem.item_id, order.quantity, beforeSnapshot);
      }
    ...
    if (statusCode === 503) return publicError(503, 'Zoho Books is temporarily busy. Please try again shortly.', requestId, retryHeaders);
    ...
    return publicError(statusCode, statusCode >= 500 ? 'Unable to process the request.' : error.message, requestId, retryHeaders);
  }
};
