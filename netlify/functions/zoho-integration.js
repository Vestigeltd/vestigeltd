...
async function getStockLockStore() {
  if (!stockLockStorePromise) {
    stockLockStorePromise = getBlobModule()
      .then(({ getStore }) => wrapCheckoutStore(getStore({ name: 'vestige-stock-locks', consistency: 'strong' })))
      .catch(error => { stockLockStorePromise = null; throw checkoutStorageError(error); });
  }
  return stockLockStorePromise;
}
async function getCheckoutStore() {
  if (!checkoutStorePromise) {
    checkoutStorePromise = getBlobModule()
      .then(({ getStore }) => wrapCheckoutStore(getStore({ name: 'vestige-checkouts', consistency: 'strong' })))
      .catch(error => { checkoutStorePromise = null; throw checkoutStorageError(error); });
  }
  return checkoutStorePromise;
}
async function getReservationStore() {
  if (!reservationStorePromise) {
    reservationStorePromise = getBlobModule()
      .then(({ getStore }) => wrapCheckoutStore(getStore({ name: 'vestige-stock-reservations', consistency: 'strong' })))
      .catch(error => { reservationStorePromise = null; throw checkoutStorageError(error); });
  }
  return reservationStorePromise;
}
async function testAtomicBlobStore(store, label) {
  ...
  try {
    ...
    if (!created?.modified || !created?.etag) throw new Error(`Atomic Blob create was not confirmed for ${label}.`);
    const read1 = await store.getWithMetadata(key, { type: 'json', consistency: 'strong' });
    if (!read1?.data || read1.data.nonce !== first.nonce || !read1.etag) throw new Error(`Strong Blob read was not confirmed for ${label}.`);
    const second = { ...first, phase: 2 };
    const updated = await store.setJSON(key, second, { onlyIfMatch: read1.etag });
    if (!updated?.modified || !updated?.etag) throw new Error(`Conditional Blob update was not confirmed for ${label}.`);
    const read2 = await store.getWithMetadata(key, { type: 'json', consistency: 'strong' });
    if (!read2?.data || read2.data.phase !== 2) throw new Error(`Updated Blob state was not visible with strong consistency for ${label}.`);
    return { ok: true, strongConsistency: true, atomicConditionalWrites: true };
  } finally {
...
async function testCheckoutStorage() {
  try {
    const [checkouts, reservations, locks] = await Promise.all([
      getCheckoutStore(),
      getReservationStore(),
      getStockLockStore(),
    ]);
    ...
    return { ok: true, strongConsistency: true, atomicConditionalWrites: true, stores };
  } catch (cause) {
    const e = new Error('Netlify Blob checkout storage failed its atomic read/write test.');
    e.statusCode = 503;
    e.service = 'checkout_storage';
    e.cause = cause;
    throw e;
...
function wrapCheckoutStore(store) {
...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    if (statusCode === 503) {
      const message = error.service === 'checkout_storage'
      ...
      return publicError(503, message, requestId, retryHeaders);
    }
