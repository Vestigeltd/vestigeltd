...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    if (statusCode >= 500) {
      ...
      let diagnosticCode = 'INTERNAL_ERROR';
      let diagnosticMessage = 'The test deployment hit an internal server error.';
      ...
      if (missing) {
        ...
        diagnosticMessage = 'Zoho Books API rejected or failed the request. Check organisation ID, API permissions, data centre, and item mappings.';
