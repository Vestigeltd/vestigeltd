...
const CHECKOUT_PAYMENT_WINDOW_MS = 30 * 60 * 1000;
...
async function findReservation(checkoutId){
  const result=await requireDatabase().prepare(
  ...
  for(const row of (result?.results||[])){
    ...
    if(reservations.some(r=>String(r?.checkoutId||'')===String(checkoutId))) return {row,data};
  }
  return null;
}
...
async function releaseReservation(checkoutId){
  for(let attempt=0;attempt<6;attempt+=1){
    const found=await findReservation(checkoutId);
    if(!found) return {released:false,reason:'not_found'};
    ...
    if(after.length===before.length) return {released:false,reason:'not_found'};
    ...
    if(Number(result?.meta?.changes||0)>0) return {released:true};
  }
  throw new Error('Unable to release website stock reservation safely after repeated D1 conflicts.');
}
...
exports.handler=async function(){
  const stats={scanned:0,voided:0,alreadyVoided:0,skippedPaid:0,manualReview:0,reservationsReleased:0,expiredMarked:0};
  try{
    ...
    for(const row of rows){
      ...
      if(!invoiceId){ stats.manualReview++; console.warn('Expired checkout has no invoice ID; manual review required',{checkoutId}); continue; }
    ...
    console.log('Vestige direct-invoice expired checkout cleanup complete',stats);
