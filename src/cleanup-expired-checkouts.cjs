...
exports.handler=async function(){
  const stats={scanned:0,voided:0,alreadyVoided:0,skippedPaid:0,manualReview:0,bankExpired:0,reservationsReleased:0,expiredMarked:0};
  try{
    ...
    for(const row of rows){
      ...
      if(paymentMode==='bank_transfer' && !invoiceId){
        // Bank/EFT checkout: no Zoho invoice exists before payment. Respect the explicit
        // payment deadline, then release only the website reservation and expire checkout.
