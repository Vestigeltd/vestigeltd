...
function assertSalesOrderMatches(salesOrder, order, customer) {
  ...
  if (Math.abs(Number(salesOrder.shipping_charge || 0) - DELIVERY_PRICE_ZAR) > PAYMENT_EPSILON) {
    const e = new Error('Recovered Sales Order shipping charge does not match the fixed R60.00 website delivery fee. Manual review is required.'); e.statusCode = 409; throw e;
  }
  if (salesOrder.delivery_method && String(salesOrder.delivery_method) !== DELIVERY_METHOD_NAME) {
    const e = new Error('Recovered Sales Order delivery method does not match this checkout. Manual review is required.'); e.statusCode = 409; throw e;
  }
  return salesOrder;
}
async function findRecoverableSalesOrder(order, customer, progress = {}) {
  if (progress.salesOrderId) {
    try {
      const byId = await getSalesOrderById(progress.salesOrderId);
      if (byId && !isVoidedStatus(byId.status)) return assertSalesOrderMatches(byId, order, customer);
    } catch (error) {
  ...
  return assertSalesOrderMatches(full, order, customer);
}
...
async function createSalesOrder(order, customer, productItem, reference, stockSnapshot) {
...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    if (body.action === 'connection_test') {
      ...
      return json(200, {
        success: true,
        message: 'Zoho Books, BC10000 stock mapping, fixed R60 delivery charge and secure checkout storage confirmed.',
    ...
    try {
      ...
      // concurrent Cloudflare Worker instances. A five-minute lease is renewed during
      ...
      salesOrder = await findRecoverableSalesOrder(order, customer, checkoutAttempt.progress || {});
      ...
      if (!salesOrder) {
        ...
        await renewDistributedLock(stockLock);
        salesOrder = await createSalesOrder(order, customer, productItem, reference, beforeSnapshot);
