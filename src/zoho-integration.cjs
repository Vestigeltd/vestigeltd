...
function ownerAlertEmail() {
  return cleanText(runtimeEnv('OWNER_ALERT_EMAIL'), 180).toLowerCase()
    || 'vestigeltd.mp@gmail.com';
}
function ownerAlertFromEmail() {
  return cleanText(runtimeEnv('OWNER_ALERT_FROM_EMAIL'), 180).toLowerCase()
    || 'orders@vestigeltd.co.za';
}
...
async function notifyOwnerOnce(kind, paymentReference, subject, text) {
  ...
  try {
    const sent = await sendOwnerEmail(subject, text);
...
async function emailPaidBankInvoice(invoiceId, order, paymentReference) {
  ...
  const body = `Dear ${customerName},<br><br>Thank you. We have received and verified your payment for Vestige order <strong>${paymentReference}</strong>.<br><br>Your paid Zoho Books invoice is attached for your records.<br><br>Regards,<br>Vestige Ltd`;
...
async function claimBankPayment(checkoutToken, requestId) {
  ...
  return {
    ...
    message: 'Payment notice sent to Vestige. Your reserved items will remain held while Vestige verifies the cleared bank credit. Your order is not confirmed until verification is complete.',
  };
}
