...
const BANK_PAYMENT_WINDOW_MS = 30 * 60 * 1000;
...
const DELIVERY_METHOD_NAME = 'The Courier Guy - Locker to Locker';
const COLLECTION_METHOD_NAME = 'Collection from Vestige Ltd';
const DELIVERY_METHOD_COURIER = 'courier_locker';
...
function ownerConsoleUrl() {
  return cleanText(runtimeEnv('OWNER_CONSOLE_URL'), 240)
    || 'https://vestigeltd.co.za/owner.html';
}
const CONTACT_EMAIL = 'contact@vestigeltd.co.za';
...
async function sendOwnerEmail(subject, text, idempotencyKey = '') {
  ...
  if (!response.ok) {
    const message =
      cleanText(data?.message, 240) ||
      cleanText(data?.error?.message, 240) ||
...
async function notifyOwnerOnce(kind, paymentReference, subject, text) {
  ...
  const claimed = existing
    ? await store.setJSON(key, sending, { onlyIfMatch: existing.etag })
    : await store.setJSON(key, sending, { onlyIfNew: true });
  if (!claimed?.modified) {
  ...
  try {
    ...
    if (!sent.sent) {
      await store.setJSON(key, {
        status: 'failed', kind: safeKind, paymentReference: ref || null,
        failedAt: Date.now(), message: cleanText(sent.message, 240) || 'Email delivery is not configured.'
...
async function writeAuditEvent(event) {
  try {
    ...
    // Auditing must never turn a successful checkout/admin action into a failed business operation.
    return false;
  }
}
...
function classifyOwnerException(order, reservations) {
  ...
  const reservationMissing = Array.isArray(order?.items) && order.items.length > 0
    ? activeReservations.some(r => !r.reservationActive)
    : false;
  ...
  if (state === 'pending_payment' && Number(order?.paymentClaimedAt || 0) > 0) {
    return {
      ...
      message: expiresAt > 0 && expiresAt <= now
        ? 'The customer says payment was sent. The stock remains on OWNER PAYMENT REVIEW HOLD even though the original timer has ended. Verify the cleared bank credit, then confirm payment or void the unpaid order.'
        : 'The customer says payment was sent. The reserved stock is now held for owner payment review. Verify the cleared bank credit, then confirm payment or void the unpaid order.'
    };
  }
  ...
  if (state === 'pending_payment' && expiresAt > now && expiresAt - now <= 5 * 60 * 1000) {
    return {
      severity: 'warning',
      code: 'EXPIRING_SOON',
...
function orderFingerprint(order) {
  ...
  const stable = JSON.stringify({
    ...
    amount: order.amount,
    items: Array.isArray(order.items) ? order.items.map(item => ({ flavour: item.flavour, itemId: item.itemId, quantity: item.quantity })) : undefined,
  });
...
function activeReservationRows(data, now = Date.now()) {
  return (Array.isArray(data?.reservations) ? data.reservations : []).filter(row =>
    ...
    (row.paymentReviewHold === true || Number(row.expiresAt || 0) > now)
  );
}
...
async function placePaymentReviewHold(itemId, checkoutId, requiredQuantity, claimedAt) {
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    const current = await store.getWithMetadata(key, { type: 'json', consistency: 'strong' });
    if (!current) {
      const e = new Error('The stock reservation could not be found for payment review. Do not assume the item is held.');
      e.statusCode = 409;
      throw e;
    }
    ...
    if (!target || Number(target.quantity || 0) < Number(requiredQuantity || 0)) {
      const e = new Error('The original stock reservation is no longer available for payment review. Owner verification is required.');
      e.statusCode = 409;
      throw e;
    }
...
async function releaseWebsiteReservationStrict(itemId, checkoutId) {
  ...
  await mutateWebsiteReservations(itemId, rows =>
    rows.filter(row => String(row.checkoutId) !== String(checkoutId))
  );
}
...
function validateBankCartOrder(input, { trustedStored = false } = {}) {
  ...
  const courierLocker = deliveryMethod === DELIVERY_METHOD_COLLECTION
    ? 'Collection'
    : validateCourierLocker(input.courierLocker);
  const checkoutId = validateCheckoutId(input.checkoutId);
  ...
  const items = rawItems.map(raw => {
    ...
    seenFlavours.add(flavour); seenItemIds.add(itemId);
...
async function requireNonNegativeStockAfterCommit(flavour, expectedItemId = '', orderedQty = 1) {
  ...
  // After a direct invoice is created, Zoho may reflect committed inventory.
  // Negative available stock is never allowed.
...
function assertDirectInvoiceMatches(invoice, order, customer, reference) {
  if (!invoice?.invoice_id) { const e = new Error('Recovered Zoho invoice is incomplete.'); e.statusCode = 409; throw e; }
  if (isVoidedStatus(invoice.status)) { const e = new Error('Recovered Zoho invoice has been voided.'); e.statusCode = 409; throw e; }
  if (String(invoice.reference_number || '') !== String(reference)) { const e = new Error('Recovered invoice reference does not match this checkout.'); e.statusCode = 409; throw e; }
  if (customer?.contact_id && String(invoice.customer_id || '') !== String(customer.contact_id)) { const e = new Error('Recovered invoice belongs to a different customer. Manual review is required.'); e.statusCode = 409; throw e; }
  ...
  if (productLines.length !== 1 || Math.abs(Number(productLines[0].quantity) - Number(order.quantity)) > PAYMENT_EPSILON || Math.abs(Number(productLines[0].rate) - PRODUCT_PRICE_ZAR) > PAYMENT_EPSILON) {
    const e = new Error('Recovered invoice product details do not match this checkout. Manual review is required.'); e.statusCode = 409; throw e;
  }
  if (Math.abs(Number(invoice.shipping_charge || 0) - DELIVERY_PRICE_ZAR) > PAYMENT_EPSILON) {
    const e = new Error('Recovered invoice shipping charge does not match the fixed R60.00 website delivery fee. Manual review is required.'); e.statusCode = 409; throw e;
  }
  return invoice;
}
async function findRecoverableDirectInvoice(order, customer, reference, progress = {}) {
  ...
  const summaries = (Array.isArray(data.invoices) ? data.invoices : []).filter(inv =>
    String(inv.reference_number || '') === String(reference) && !isVoidedStatus(inv.status)
  );
  if (summaries.length > 1) { const e = new Error('Multiple active Zoho invoices exist for this checkout reference. Manual review is required.'); e.statusCode = 409; throw e; }
...
async function allocateBankPaymentReference() {
  ...
  for (let attempt = 0; attempt < 12; attempt += 1) {
    ...
    if (!Number.isSafeInteger(currentValue) || currentValue < 0) {
      const e = new Error('Order reference sequence is invalid.'); e.statusCode = 503; e.service = 'checkout_storage'; throw e;
    }
    const nextValue = currentValue + 1;
    if (nextValue > 99999999) {
      const e = new Error('Order reference sequence is exhausted.'); e.statusCode = 503; e.service = 'checkout_storage'; throw e;
    }
  ...
  const e = new Error('Unable to allocate a unique order reference safely.'); e.statusCode = 503; e.service = 'checkout_storage'; e.retryAfter = '2'; throw e;
}
function publicEftDetails() {
  const pick = (name) => cleanText(runtimeEnv(name), 120);
  const details = {
...
async function locateBankCheckoutByReference(paymentReference) {
  ...
  // Compatibility fallback for bank checkouts created before v34 introduced the
  // dedicated payment-reference index. The result is then indexed for future calls.
  const db = requireDatabase();
  const result = await db.prepare(
  ...
  for (const row of (Array.isArray(result?.results) ? result.results : [])) {
    let data;
    try { data = JSON.parse(row.value_json); } catch (_) { continue; }
...
function assertBankInvoiceMatches(invoice, order, customer, paymentReference) {
  if (!invoice?.invoice_id) { const e = new Error('Recovered Zoho invoice is incomplete.'); e.statusCode = 409; throw e; }
  if (isVoidedStatus(invoice.status)) { const e = new Error('Recovered Zoho invoice has been voided.'); e.statusCode = 409; throw e; }
  if (String(invoice.reference_number || '') !== String(paymentReference)) { const e = new Error('Recovered invoice reference does not match this payment.'); e.statusCode = 409; throw e; }
  if (String(invoice.customer_id || '') !== String(customer.contact_id || '')) { const e = new Error('Recovered invoice belongs to a different customer. Manual review is required.'); e.statusCode = 409; throw e; }
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) { const e = new Error('Recovered invoice basket does not match this checkout. Manual review is required.'); e.statusCode = 409; throw e; }
  if (Math.abs(Number(invoice.shipping_charge || 0) - Number(order.deliveryCharge || 0)) > PAYMENT_EPSILON) { const e = new Error('Recovered invoice fulfilment charge does not match this checkout. Manual review is required.'); e.statusCode = 409; throw e; }
  if (Math.abs(Number(invoice.total || 0) - Number(order.amount)) > PAYMENT_EPSILON) { const e = new Error('Recovered invoice total does not match the verified bank payment amount. Manual review is required.'); e.statusCode = 409; throw e; }
  return invoice;
}
async function findBankInvoice(order, customer, paymentReference, progress = {}) {
  ...
  if (matches.length > 1) { const e = new Error('Multiple active Zoho invoices use this payment reference. Manual review is required.'); e.statusCode = 409; throw e; }
...
function assertBankPaymentMatches(payment, customer, invoice, paymentReference, amount) {
  if (!payment?.payment_id) { const e = new Error('Recovered Zoho customer payment is incomplete.'); e.statusCode = 409; throw e; }
  if (String(payment.reference_number || '') !== paymentReference) { const e = new Error('Recovered customer payment reference does not match. Manual review is required.'); e.statusCode = 409; throw e; }
  if (String(payment.customer_id || '') !== String(customer.contact_id || '')) { const e = new Error('Recovered customer payment belongs to a different customer. Manual review is required.'); e.statusCode = 409; throw e; }
  if (String(payment.payment_mode || '').toLowerCase() !== 'banktransfer') { const e = new Error('Recovered customer payment uses a different payment mode. Manual review is required.'); e.statusCode = 409; throw e; }
  if (Math.abs(Number(payment.amount || 0) - Number(amount)) > PAYMENT_EPSILON) { const e = new Error('Recovered customer payment amount does not match the confirmed bank credit. Manual review is required.'); e.statusCode = 409; throw e; }
  ...
  if (Math.abs(appliedTotal - Number(amount)) > PAYMENT_EPSILON) { const e = new Error('Recovered customer payment is not fully applied to the expected invoice. Manual review is required.'); e.statusCode = 409; throw e; }
  return payment;
}
...
async function findBankCustomerPayment(customer, invoice, paymentReference, amount, progress = {}) {
  if (progress.bankPaymentId) {
    try { return assertBankPaymentMatches(await getCustomerPayment(progress.bankPaymentId), customer, invoice, paymentReference, amount); }
    catch (error) { if (error.statusCode === 409) throw error; }
  }
  ...
  if (matches.length > 1) { const e = new Error('Multiple Zoho customer payments use this payment reference. Manual review is required.'); e.statusCode = 409; throw e; }
  if (!matches.length) return null;
...
async function createBankCustomerPayment(customer, invoice, paymentReference, amount, paymentDate) {
  ...
  if (accountId) {
    if (!/^\d+$/.test(accountId)) { const e = new Error('ZOHO_BANK_ACCOUNT_ID must be a numeric Zoho Books account ID.'); e.statusCode = 503; throw e; }
...
async function beginBankConfirmationState(located) {
  ...
  if (!latest) { const e = new Error('Checkout could not be found before confirmation.'); e.statusCode = 404; throw e; }
  const state = String(latest.data?.state || '');
  if (state === 'confirmed' || state === 'confirming_payment') return latest.data;
  if (state !== 'pending_payment') { const e = new Error('Checkout is no longer pending payment. Manual review is required.'); e.statusCode = 409; throw e; }
  const record = {
    ...latest.data,
    ...
    progress: {
      ...
      ...(Number(latest.data?.progress?.paymentExpiresAt || 0) <= Date.now()
        ? { paidAfterExpiryRecoveredAt: new Date().toISOString() }
        : {}),
    },
    updatedAt: Date.now(),
  ...
  if (!result?.modified) { const e = new Error('Checkout confirmation state changed concurrently. Please retry.'); e.statusCode = 503; e.retryAfter = '2'; throw e; }
  return record;
}
async function strictlyConfirmBankCheckout(located, invoice, payment, paymentDate, amount) {
  const latest = await located.store.getWithMetadata(located.key, { type: 'json', consistency: 'strong' });
  if (!latest) { const e = new Error('Checkout disappeared before confirmation could be recorded.'); e.statusCode = 409; throw e; }
  if (String(latest.data?.state || '') === 'confirmed') return latest.data;
  if (!['pending_payment', 'confirming_payment'].includes(String(latest.data?.state || ''))) { const e = new Error('Checkout is no longer eligible for payment confirmation. Manual review is required.'); e.statusCode = 409; throw e; }
  const record = {
    ...latest.data,
    state: 'confirmed',
    progress: {
  ...
  const result = await located.store.setJSON(located.key, record, { onlyIfMatch: latest.etag });
  if (!result?.modified) { const e = new Error('Checkout confirmation state changed concurrently. Retry the same payment reference; Zoho recovery prevents duplicate financial records.'); e.statusCode = 503; e.retryAfter = '2'; throw e; }
  return record;
}
...
async function adminLookupBankOrder(paymentReference) {
  ...
  return {
    ...
    reservations,
    cancellable:
      ['pending_payment', 'cancelling_customer'].includes(summary.state),
    confirmable:
      ['pending_payment', 'confirming_payment'].includes(summary.state),
  };
}
  ...
  try {
    ...
    const cancelling = {
      ...
      progress: {
        ...
        adminCancellationStartedAt: new Date().toISOString(),
      },
      updatedAt: Date.now(),
    };
...
async function confirmBankPaymentManually(input) {
  ...
  if (!Number.isFinite(actualAmount) || actualAmount <= 0) { const e = new TypeError('A valid confirmed bank-credit amount is required.'); e.statusCode = 400; throw e; }
  const paymentDate = validatePaymentDate(input.paymentDate);
  if (input.bankCreditConfirmed !== true) { const e = new TypeError('Explicit bank-credit confirmation is required.'); e.statusCode = 400; throw e; }
  ...
  try {
    ...
    if (!['pending_payment', 'confirming_payment'].includes(String(data.state || ''))) { const e = new Error('This checkout is not eligible for bank-payment confirmation. Manual review is required.'); e.statusCode = 409; throw e; }
    if (String(data.progress?.paymentMode || data.response?.paymentMode || '') !== 'bank_transfer') { const e = new Error('This checkout is not a bank-transfer checkout.'); e.statusCode = 409; throw e; }
    const paidAfterExpiry = String(data.state || '') === 'pending_payment'
      && Number(data.progress?.paymentExpiresAt || 0) <= Date.now();

    const order = storedBankOrderFromCheckout(located.checkoutId, data);
    if (Math.abs(actualAmount - Number(order.amount)) > PAYMENT_EPSILON) { const e = new Error(`Confirmed bank credit must equal the exact order total of ${moneyForError(order.amount)}.`); e.statusCode = 409; throw e; }
    ...
    for (const line of order.items) {
      ...
      if (!snapshot.canFulfil) {
        const e = new Error(
          paidAfterExpiry
            ? `${line.flavour} is no longer safely available for this already-paid expired order. Do not cancel or create another order; manual fulfilment review is required.`
            : `${line.flavour} can no longer be fulfilled safely. Do not auto-confirm this bank payment.`
        );
        e.statusCode = 409;
        throw e;
      }
      ...
      if (paidAfterExpiry) {
        // The bank credit is already visibly cleared and the original website hold
        // expired while awaiting owner confirmation. Re-create a conservative website
        // reservation only after fresh Zoho stock + reservation-overlay validation and
        // while the deterministic stock lock is held.
    ...
    // Re-read durable progress before financial writes so a prior partial attempt can
    // recover its Zoho IDs. Search-by-reference also covers lost POST responses.
    ...
    if (Math.abs(Number(finalInvoice.balance || finalInvoice.balance_amount || 0)) > PAYMENT_EPSILON && String(finalInvoice.status || '').toLowerCase() !== 'paid') {
      const e = new Error('Zoho payment was created but the invoice is not fully settled. Manual review is required.'); e.statusCode = 409; throw e;
    }
...
function moneyForError(value) { return `R${Number(value || 0).toFixed(2)}`; }
  ...
  try {
    ...
    if (currentState !== 'cancelled_customer' && currentState !== 'cancelling_customer') {
      const cancelling = {
        ...
        progress: {
          ...(latest.data?.progress || {}),
          customerCancellationStartedAt: new Date().toISOString(),
        },
        updatedAt: Date.now(),
      };
    ...
    if (String(fresh.data?.state || '') !== 'cancelled_customer') {
      const cancelled = {
        ...
        progress: {
          ...(fresh.data?.progress || {}),
          customerCancelledAt: new Date().toISOString(),
...
async function getBankCheckoutStatus(token) {
  ...
  if (checkout.paymentMode !== 'bank_transfer') {
    const e = new Error('This checkout token is not a bank-payment checkout.'); e.statusCode = 400; throw e;
  }
  const store = await getCheckoutStore();
  const current = await store.getWithMetadata(`checkout-${checkout.checkoutId}`, { type: 'json', consistency: 'strong' });
  if (!current) { const e = new Error('Checkout could not be found.'); e.statusCode = 404; throw e; }
  const state = String(current.data?.state || '');
  ...
  try {
    ...
    // Checkout status must remain available even if notification-state lookup fails.
  }
  return {
    ...
    paymentStatus:
      ...
      state === 'cancelled_customer' || state === 'cancelled_unpaid' ? 'cancelled' :
      'pending',
  };
}
async function claimBankPayment(checkoutToken, requestId) {
  ...
  // First persist the customer payment claim. Scheduled cleanup will treat this durable
  // claim as an owner-review hold and will not expire/release the checkout.
  ...
  // Re-read after the durable claim so retries are idempotent and cleanup cannot race
  // the owner-review transition.
  current = await located.store.getWithMetadata(located.key, { type: 'json', consistency: 'strong' });
  if (!current) {
  ...
  // Record that every line is now protected by an owner-review hold.
  ...
  await writeAuditEvent({
    ...
    message: notification.sent
      ? 'Customer reported payment sent; stock placed on owner payment review hold; owner alert delivered.'
      : 'Customer reported payment sent; stock placed on owner payment review hold; owner alert could not be delivered.',
    requestId,
  ...
  return {
    ...
    message: notification.sent
      ? 'Vestige has been notified that you sent payment. Your reserved items will remain held while the cleared bank credit is verified. Your order is not confirmed until verification is complete.'
      : notification.pending
        ? 'Your payment notice is recorded and the owner alert is still being processed. Your reserved items remain held. If this message persists, press the payment notice button again.'
        : 'Your payment notice is recorded and your reserved items remain held, but the automatic owner alert could not be confirmed. Please press the payment notice button again or contact Vestige Ltd. Your order is not yet confirmed.',
  };
}
async function prepareBankOrder(order, requestId) {
  ...
  try {
    // Preflight every line before taking distributed locks. Item identity must match
    // the verified Zoho catalogue mapping supplied by the availability endpoint.
    for (const line of order.items) {
      ...
      if (String(preflight.item.item_id) !== String(line.itemId)) {
        const e = new Error(`${line.flavour} no longer matches the Zoho Books item catalogue. Please refresh the shop.`); e.statusCode = 409; throw e;
      }
    }

    // Acquire item locks in deterministic item-id order so two multi-line baskets
    // cannot deadlock each other by locking the same flavours in different orders.
    const lockLines = [...order.items].sort((a, b) => String(a.itemId).localeCompare(String(b.itemId)));
    ...
    // Re-read stock after all locks are held, then apply the website-reservation
    // overlay. No reservation is written until every basket line can be fulfilled.
    const finalLines = [];
    for (const line of order.items) {
      ...
      if (!snapshot.canFulfil) {
        const e = new Error(snapshot.stock === 0
          ? `${line.flavour} has just been reserved by another customer and is now unavailable.`
          : `Only ${snapshot.stock} unit(s) of ${line.flavour} remain after active website reservations. Please lower that quantity or remove the flavour.`);
        e.statusCode = 409; e.freshAvailabilityNeeded = true; throw e;
      }
      finalLines.push({ line, snapshot });
    }

    // Reserve all basket lines under the same checkout id. If any later operation
    // fails, the catch block releases every line already written.
    ...
    const responsePayload = {
      ...
      checkoutToken,
      paymentExpiresAt,
      expiresInMinutes: Math.round(BANK_PAYMENT_WINDOW_MS / 60000),
...
async function verifyDirectInvoiceAtPayment(checkout) {
  ...
  if (!invoice || String(invoice.invoice_id || '') !== String(checkout.invoiceId)) {
    const e = new Error('Invoice could not be re-verified before confirmation.'); e.statusCode = 409; throw e;
  }
  if (isVoidedStatus(invoice.status)) { const e = new Error('The invoice has been voided and cannot be confirmed.'); e.statusCode = 409; throw e; }
  if (String(invoice.reference_number || '') !== webReference(checkout.checkoutId)) {
    const e = new Error('Invoice checkout reference no longer matches.'); e.statusCode = 409; throw e;
  }
  const lines = Array.isArray(invoice.line_items) ? invoice.line_items : [];
  ...
  if (productLines.length !== 1 || Math.abs(Number(productLines[0].quantity) - Number(checkout.quantity)) > PAYMENT_EPSILON || Math.abs(Number(productLines[0].rate) - PRODUCT_PRICE_ZAR) > PAYMENT_EPSILON) {
    const e = new Error('Invoice product details changed after checkout and require manual review.'); e.statusCode = 409; throw e;
  }
  if (Math.abs(Number(invoice.shipping_charge || 0) - DELIVERY_PRICE_ZAR) > PAYMENT_EPSILON) { const e = new Error('Invoice delivery charge changed after checkout.'); e.statusCode = 409; throw e; }
  return invoice;
}
...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    if (body.action === 'admin_order_lookup') {
      ...
      const order = await adminLookupBankOrder(body.paymentReference);
      return json(200, { success: true, order, requestId });
    }
    ...
    if (body.action === 'prepare_bank_order') {
      const order = validateBankCartOrder(body);
      const prepared = await prepareBankOrder(order, requestId);
      await writeAuditEvent({
    ...
    try {
      ...
      // Direct-invoice architecture: D1 is the authoritative short-lived website
      // reservation ledger. Zoho invoice integrity is verified before payment link
      // exposure and again at payment confirmation.
    ...
    if (statusCode >= 500) {
      ...
      let diagnosticCode = diagnosticStage && diagnosticStage !== 'COMPLETE'
        ? (checkoutDiagnostic ? `${diagnosticStage}_FAILED` : `CONNECTION_TEST_${diagnosticStage}_FAILED`)
        : 'INTERNAL_ERROR';
      let diagnosticMessage = diagnosticStage && diagnosticStage !== 'COMPLETE'
        ? (checkoutDiagnostic
          ? `The controlled checkout test failed during stage: ${diagnosticStage}.`
