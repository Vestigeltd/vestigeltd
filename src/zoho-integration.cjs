...
async function getBankCheckoutStatus(token) {
  ...
  return {
    ...
    state,
    paymentStatus: state === 'confirmed' ? 'confirmed' : state === 'expired' ? 'expired' : 'pending',
  };
}
async function prepareBankOrder(order, requestId) {
  ...
  try {
    ...
    const snapshot = await applyWebsiteReservationOverlay(finalStock.snapshot, order.itemId, order.checkoutId);
    if (!snapshot.canFulfil) {
      const e = new Error(snapshot.stock === 0
        ? 'This flavour has just been reserved by another customer and is now unavailable.'
        : `Only ${snapshot.stock} unit(s) remain after active website reservations. Please lower the quantity or choose another flavour.`);
      e.statusCode = 409; e.freshAvailabilityNeeded = true; throw e;
    }

    await addWebsiteReservation(order, snapshot, null);
    ...
    const responsePayload = {
      ...
      message: 'Stock reserved for 30 minutes. Pay the exact total by Capitec QR Pay or EFT. Your order remains pending until the actual bank payment is verified.',
      capitec: {
        available: true,
    ...
    // No bank payment can be made until the instructions are successfully returned,
    // so a failed preparation can safely release its temporary website reservation.
    await releaseWebsiteReservation(order.itemId, order.checkoutId);
